...
from alltime_athletics_viz.utils.logger import logger
...
def rename_df_list_elements(df_list: list[pl.DataFrame]) -> list[pl.DataFrame]:
    ...
    return [
        df.with_columns([pl.col(c).cast(str) for c in df.columns])
        .rename(
            mapping=decide_mapping_men(
                df["event"].unique()[0], len(df.columns), mappings
            )
        )
        .with_columns(
            pl.col("result").apply(convert_time_to_seconds).alias("result seconds")
        )
        .with_columns(pl.col("rank").cast(int))
        for df in df_list
    ]
